...
try {
  ...
  /* --- две поверхности: рабочий стол внизу, панель наверху --- */
  /* Поднять настоящий оконный сервер здесь нечем, но всё, что решает
     страница, проверяется и так: кем она себя считает и что прячет. */
  const поверхности = await (async () => {
    const снять = async адрес => {
      const p4 = await browser.newPage({ viewport:{ width:1280, height:800 } });
      const беды = [];
      p4.on('pageerror', e => беды.push(e.message));
      try {
        await p4.goto(адрес);
        await p4.waitForTimeout(2400);
        return await p4.evaluate(() => {
          const видно = с => {
            const н = document.querySelector(с);
            if (!н) return false;
            const st = getComputedStyle(н);
            return st.display !== 'none' && st.visibility !== 'hidden';
          };
          return { кто:window.Поверхности ? Поверхности.кто : 'нет',
                   раздельно:window.Поверхности ? Поверхности.разделено() : false,
                   обои:видно('#wallpaper'), значки:видно('#desktop-icons'),
                   верхняя:видно('.topbar'), панель:видно('.dock-wrap'), окна:видно('#windows') };
        }).then(о => ({ ...о, беды }));
      } finally { await p4.close(); }
    };
    return {
      один:  await снять(URL_APP),
      стол:  await снять(URL_APP + (URL_APP.includes('?') ? '&' : '?') + 'surface=desktop&раздельно=1'),
      бар:   await снять(URL_APP + (URL_APP.includes('?') ? '&' : '?') + 'surface=панель&раздельно=1')
    };
  })();

  check('без разделения всё остаётся на одной поверхности',
    поверхности.один.кто === 'стол' && поверхности.один.раздельно === false
      && поверхности.один.панель === true && поверхности.один.обои === true,
    JSON.stringify(поверхности.один));
  check('разделённый стол отдаёт панель наверх, а себе оставляет обои и окна',
    поверхности.стол.кто === 'стол' && поверхности.стол.раздельно === true
      && поверхности.стол.панель === false && поверхности.стол.обои === true
      && поверхности.стол.окна === true,
    JSON.stringify(поверхности.стол));
  check('панель рисует только себя, без обоев и чужих окон',
    поверхности.бар.кто === 'панель' && поверхности.бар.панель === true
