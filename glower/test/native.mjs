...
try {
  ...
  /* Значка «настоящий диск» в трее больше нет: он сообщал то, что верно всё
     время работы системы, и занимал место, нужное значкам самих программ.
     Проверяем обратное — что его действительно не осталось, и что связь с
     системой видна там, где ей и место. */
  check('значка «настоящий диск» в трее не осталось',
    (await page.$$('#agent-badge')).length === 0);
  check('оболочка знает, что работает с настоящей системой',
    await page.evaluate(() => document.body.classList.contains('native')));
  ...
  await page.evaluate(() => FS.write(['Документы'], 'из-оболочки.txt', 'создано оболочкой GlowerOS'));
  await page.waitForTimeout(900);
  const written = existsSync(join(WS, 'Документы', 'из-оболочки.txt'))
    && (await readFile(join(WS, 'Документы', 'из-оболочки.txt'), 'utf8')).includes('создано оболочкой GlowerOS');
