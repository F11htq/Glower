...
const Web = {
  ...
  async engine(){
    ...
    this.starting = (async () => {
      ...
      /* Движок может не подняться и на машине — тогда рядом стоящий
         настоящий браузер лучше пустого окна с объяснением. */
...
/* ==========================================================================
   Приложение «Браузер»
   ========================================================================== */
/* На машине с настоящим браузером открываем его, а не себя.

   Наш браузер — это окно вокруг движка, который живёт на машине. Когда
   движка нет, показывать пустое окно с объяснением бессмысленно: рядом
   стоит Firefox, который умеет всё то же самое и лучше.

   Раньше так и выходило: человек нажимал «Браузер» при первом запуске,
   видел сообщение, что движка нет, закрывал окно — и только тогда
   появлялся Firefox. Подмену списка программ на машинные делает отдельный
   обход, и до первого нажатия он не всегда успевает. Ждать его незачем:
   спросим прямо сейчас.  */
async function машинныйБраузер(){
  if (!window.OS || !OS.on()) return false;
  /* Спрашиваем машину напрямую, а не свой список программ: список
     заполняется обходом, и до первого нажатия он может быть ещё пуст. */
  for (const ярлык of ['firefox.desktop', 'firefox-esr.desktop',
                       'org.gnome.Epiphany.desktop', 'chromium.desktop',
                       'chromium-browser.desktop', 'google-chrome.desktop']){
    try {
      const о = await Platform.rpc('sys.launch', { id:ярлык });
      if (о && о.ok !== false) return true;
    } catch(e){ /* нет такой — пробуем следующую */ }
  }
  return false;
}
...
APPS.browser = {
  ...
  render(win){
    /* Прежде всего — есть ли чем рисовать страницы. Если движок не
       поднимается, отдаём дело настоящему браузеру машины и закрываемся
       сами, не показывая человеку окно с извинениями. */
    (async () => {
      try { await Web.engine(); }
      catch(е){
        if (await машинныйБраузер()){
          try { WM.close(win); } catch(_){}
          Shell.toast('Браузер', 'Открываю браузер машины', '🌐', 4000);
        }
      }
    })();
