...
const Session = {
  ...
  восстановлено:false,
  /* Короткая память о последних записях: кто сохранял, сколько окон, а если
     не сохранял — почему. Пропажа сеанса случается раз на десяток запусков,
     и без этой записи после перезагрузки не остаётся ровно ничего: список
     пуст, а кто его опустошил — неизвестно. Журнал переживает перезагрузку
     и отвечает на этот вопрос прямо. */
  помета(почему, сколько){
    try {
      const л = KV.get('session.журнал', []);
      л.push({ п:почему, с:сколько, т:Date.now() });
      while (л.length > 8) л.shift();
      KV.set('session.журнал', л);
    } catch(e){}
  },
  /* «почему» — кто позвал: «сам» (окно закрыли, открыли), «часы» (раз в
     двадцать секунд) или «уход» (страница закрывается). Автоматические
     записи не имеют права опустошить сеанс: человек ничего не закрывал. */
  save(почему = 'сам'){
    if (S.restoreSession === false) return;
    if (!this.восстановлено && !(window.WM && WM.wins.length)) return this.помета(почему + ':рано', -1);
    ...
    if (!(window.WM && WM.wins.length) && !столВидно) return this.помета(почему + ':нет стола', -1);
    /* Сеанс опустошает только человек. Таймер и уход со страницы застают
       систему в любом состоянии — в том числе в том полуторасекундном
       окне, когда окна ещё не вернулись, а «восстановлено» уже стоит. Там
       и пропадал сеанс. Пустоту от них не принимаем. */
    if (!(window.WM && WM.wins.length) && почему !== 'сам'
        && (KV.get(this.KEY, []) || []).length) return this.помета(почему + ':пусто', -1);
    ...
    this.помета(почему, list.length);
  },
  restore(){
    if (S.restoreSession === false){ this.восстановлено = true; return; }
    /* Возвращать окна за экран блокировки незачем: человек ещё не вошёл, а
       «восстановлено» уже встанет — и первая же запись по таймеру запишет
       поверх сеанса пустоту. Ждём рабочего стола; если его так и нет —
       выходим ни с чем, ничего не трогая. */
    const стол = document.querySelector('#desktop');
    if (!(стол && стол.classList.contains('on'))){
      if ((this.ждём = (this.ждём || 0) + 1) < 120) return setTimeout(() => this.restore(), 500);
      this.помета('не дождались стола', -1);
      return;
    }
...
addEventListener('beforeunload', () => Session.save('уход'));
setInterval(() => Session.save('часы'), 20000);
