...
/* ==========================================================================
   Быстрые клавиши, как в Windows: PrintScreen и Win+Shift+S
   ========================================================================== */
(function горячие(){
  const снять = async режим => {
    if (!window.Platform || Platform.mode !== 'native'){
    ...
    try {
      ...
      Shell.toast('Ножницы', 'Не вышло снять экран: ' + (e.message || e), '⚠️', 7000);
    }
  };
  const o_отменено = о => !!(о && о['отменено']);

  addEventListener('keydown', e => {
    if (e.key === 'PrintScreen'){ e.preventDefault(); снять('экран'); return; }
    if ((e.metaKey || (e.ctrlKey && e.altKey)) && e.shiftKey && e.key.toLowerCase() === 's'){
      e.preventDefault(); снять('область');
    }
  });
})();
