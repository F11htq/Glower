...
APPS.store = {
  ...
  render(win, opts){
    ...
    /* ---------- что нужно программе сверх неё самой ----------

       Steam, поставленный из Flathub, при первом запуске жалуется на
       «Missing permissions for input devices»: правила udev для геймпадов
       лежат не в нём, а в отдельном крошечном пакете системы, и поставить
       его песочница не может — это дело системы.

       Человек в этот миг видит окно с английской бранью и ссылкой на wiki
       — сразу после установки игры, которую он только что поставил у нас.
       Ставим недостающее сами: двадцать килобайт, и жалобы нет.

       Список нарочно короткий и точечный: сюда попадает только то, без
       чего программа сразу же ругается. */
    const ДОПОЛНЕНИЯ = {
      'com.valvesoftware.Steam':{ пакет:'steam-devices',
        зачем:'правила для геймпадов' }
    };

    async function доставьДополнение(имя){
      const доп = ДОПОЛНЕНИЯ[имя];
      if (!доп) return;
      try {
        const стоит = await Pkg.installed().catch(() => null);
        const список = (стоит && (стоит.list || стоит)) || [];
        if (Array.isArray(список) && список.some(п => (п.name || п) === доп.пакет)) return;
        await Pkg.install(доп.пакет, 'apt');
        Shell.toast('Магазин', 'Доставляю ' + доп.зачем + '…', '🎮', 6000);
      } catch(e){ /* не вышло — программа всё равно работает, просто поворчит */ }
    }

    async function следиЗаРаботой(ставим){
      for (let i = 0; i < 900; i++){
        let j;
        ...
        if (!j.running){
          Shell.toast('Магазин',
            ...
            j.ok ? '✅' : '⚠️', 8000);
          /* Доставку зовём после того, как работа кончилась: две установки
             разом система не ведёт, и вторая просто получила бы отказ. */
          if (j.ok && j.action === 'install' && ставим) await доставьДополнение(ставим);
          break;
        }
        await new Promise(r => setTimeout(r, 1200));
      }
