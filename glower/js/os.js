...
(async function boot(){
  ...
  /* «Оболочка управляет машиной такой-то» тоже убрано. Это не новость и
     не событие: человек включил свою машину и смотрит в свой рабочий
     стол. Имя машины, кому оно нужно, лежит в «Параметрах». */
...
/* Магазин — отдельный файл и в наши переменные не заглядывает, а значок
   программы ему нужен тот же самый, настоящий. Отдаём наружу. */
OS.значокПрограммы = (a, класс) => значокМашины(a, класс);
...
async function запустиПрограмму(a){
  const скажи = () => {
    ...
    /* Подсказки «программа открылась своим окном, а вернуться можно так»
       здесь больше нет.
    
       Она была верной и совершенно бесполезной: человек и без неё видит,
       что окно открылось, а искать дорогу назад начинает тогда, когда она
       понадобится, — и тогда её нужно не в углу экрана полторы секунды
       назад, а под рукой. Каждое такое уведомление требует внимания к
       тому, что уже произошло и очевидно. */
  };
...
/* Развёрнутое чужое окно: панель прижимается к краю экрана и распрямляется
   ...
   В раздельном режиме этого не происходит вовсе: там док — островок и
   только островок. Смена формы на ходу выглядела так, будто система
   передумала: стоило развернуть окно, и док на глазах менял и форму, и
   место. А поскольку в раздельном режиме док к тому же уезжает вниз, он
   успевал сделать оба движения разом, и уход получался дёрганым. */
...
function занятЭкран(да){
  ...
  document.body.classList.toggle('впритык', да && !островком);
