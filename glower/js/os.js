...
function дайЗначок(имя){
  ...
  const было = значкиПамять.get(имя);
  ...
  const обещание = Platform.rpc('sys.icon', { имя })
    .then(d => {
      ...
      значкиПамять.set(имя, { ответ:Promise.resolve(src), есть:!!src, когда:Date.now() });
      return src;
    })
    .catch(() => null);
  значкиПамять.set(имя, { ответ:обещание, есть:false, когда:Date.now() });
  return обещание;
}
...
function wireNotifications(){
  ...
  const слушай = async () => {
    while (работаем){
      try {
        const d = await Platform.rpc('ui.hear', { с, темы:['уведомление', 'файл-установщик'] });
        с = d.n || с;
        (d.список || []).forEach(м => {
...
function следиЗаУстановкой(имя){
  ...
  const часы = setInterval(async () => {
    ...
    if (ход.running){
      const шаг = (ход.step || '') + ' · ' + (ход.percent || 0) + '%';
      if (шаг !== последнее){ последнее = шаг; Shell.toast(имя, шаг, '📦', 2500); }
      return;
    }
    clearInterval(часы);
    if (ход.ok){
