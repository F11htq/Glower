...
const Shell = {
  ...
  рисуйОбзор(){
    ...
    видно.forEach((п, i) => {
      ...
      б.oncontextmenu = e => { e.preventDefault(); this.менюПрограммы(e, п); };
      б.onclick = () => {
        this.обзор(false);
  ...
  /* Меню программы в обзоре — по правой кнопке, как везде.
  
     Раньше правая кнопка молча проваливалась на рабочий стол, и человек
     получал «Создать папку» поверх открытого меню программ. Теперь здесь
     то, что про эту самую программу: открыть (в том числе на соседнем
     столе), вынести на стол, закрепить в доке. */
  менюПрограммы(e, п){
    const машина = п['вид'] === 'машина';
    const пункты = [];

    пункты.push({ i:п['знак'] || '▢', t:'Открыть', f:() => {
      this.обзор(false);
      if (машина && window.OS && OS.запустиПоЯрлыку) OS.запустиПоЯрлыку(п.id);
      else WM.open(п.id);
    } });

    /* Открыть на соседнем столе — то же, что бросить значок на него
       мышью, только клавиатурным путём: не всем удобно тащить. */
    if (WM.desks > 1 || !машина)
      пункты.push({ i:'🖥', t:'Открыть на новом рабочем столе', f:() => {
        const куда = WM.добавьСтол();
        this.обзор(false);
        if (машина){ WM.gotoDesk(куда); if (window.OS && OS.запустиПоЯрлыку) OS.запустиПоЯрлыку(п.id); }
        else { const w = WM.open(п.id); if (w) WM.наСтол(w, куда); }
      } });

    пункты.push('hr');

    if (машина && window.OS && OS.наСтол)
      пункты.push({ i:'🖥', t:OS.наСтоле(п.id) ? 'Убрать с рабочего стола' : 'Вынести на рабочий стол',
        f:() => OS.наСтол({ id:п.id, name:п['имя'], 'значок':п['значок'] || '' }) });
    else if (window.Link && Link.toDesktop)
      пункты.push({ i:'🖥', t:'Вынести на рабочий стол',
        f:() => Link.toDesktop({ app:п.id }, п['имя']) });

    if (!машина && APPS[п.id]){
      const вДоке = (S.dockApps || []).includes(п.id);
      пункты.push({ i:'📌', t:вДоке ? 'Убрать из дока' : 'Закрепить в доке', f:() => {
        S.dockApps = вДоке ? S.dockApps.filter(x => x !== п.id) : [...S.dockApps, п.id];
        Store.save(); this.renderDock();
        this.toast('Док', вДоке ? 'Убрано из дока' : 'Закреплено в доке', '📌');
      } });
    }

    if (машина)
      пункты.push({ i:'🛍', t:'Найти в Магазине', f:() => {
        this.обзор(false);
        WM.open('store', { найти:String(п['имя'] || '').split(' ')[0] });
      } });

    this.ctx(e.clientX, e.clientY, пункты);
  },
  ...
  bindDesktop(){
    ...
    d.addEventListener('contextmenu', e => {
      /* Обзор лежит поверх стола и принадлежит ему же по разметке, поэтому
         правая кнопка на программе проваливалась вниз и открывала меню
         рабочего стола — поверх открытого меню программ. Своё меню обзор
         показывает сам; по пустому месту в нём не показывает никакого. */
      if (e.target.closest('.обзор')){ e.preventDefault(); return; }
