/* ==========================================================================
   Панель задач во всю ширину, управляемые виджеты стола,
...
const IDB = {
  ...
  open(){
    ...
    return new Promise((res, rej) => {
      const r = indexedDB.open('glower', 1);
...
/* ==========================================================================
   1. Нижняя панель: док ↔ панель задач во всю ширину
   ========================================================================== */
const dock = $('#dock');
const tray = $('#tb-tray');
const topbar = $('.topbar');
...
Shell.updateTaskbar = function(){
  ...
  /* В раздельном режиме док остаётся островком при любых окнах: форму на
     ходу он не меняет. Полосу во всю ширину человек включает сам, в
     параметрах, — тогда она и будет. */
  const островком = window.Поверхности && Поверхности.разделено && Поверхности.разделено();
  document.body.classList.toggle('taskbar', anyMax && !островком && S.taskbarFull !== false);
...
/* Размер значков панели.
   ...
   Раньше он был жёстким: 52 точки, а в режиме панели задач — не больше 42.
   На большом экране такая панель выглядит ниткой, и разглядеть на ней
   что-либо трудно. Настоящие системы считают размер от экрана — считаем и
   мы: примерно двадцатая часть высоты, но не меньше 44 и не больше 92.

   Если человек сам двигал ползунок размера, его выбор главнее любого
   расчёта: система не должна переспорить хозяина. */
Shell.размерДока = function(taskbar){
  if (S.dockSizeСвой) return S.dockSize;
  const h = Math.max(600, innerHeight || 900);
  const база = Math.round(Math.min(92, Math.max(44, h * 0.052)));
  /* В режиме панели задач значки чуть скромнее: полоса идёт во всю ширину,
     и крупные значки на ней смотрятся тяжело. */
  return taskbar ? Math.round(база * 0.84) : база;
};
...
Shell.fitDock = function(){
  ...
  let size = Shell.размерДока(taskbar);
...
$('#desktop').addEventListener('contextmenu', e => {
  /* В обзоре у правой кнопки своё меню — про программу, на которую
     нажали. Виджетам там делать нечего: они живут на столе, а стол в этот
     миг закрыт обзором. Раньше этот пункт дописывался в любое открытое
     меню, и в меню программы появлялось «Добавить виджет…». */
  if (e.target.closest('.обзор')) return;
...
/* ==========================================================================
   ...
   ========================================================================== */
const SYNTH = [
  { t:'Тихая вода', a:'Синтезатор', scale:[0,3,5,7,10], root:220, bpm:96, e:'🌊', synth:true },
