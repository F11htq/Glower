...
Shell.tellPanelHeight = function(){
  ...
  /* Док висит не вплотную к краю, а в нескольких пикселях от него — так он и
     задуман. Когда панель рисует нижний слой, эта щель принадлежит рабочему
     столу, и требовать прилегания правильно. Но у отдельной поверхности щель
     принадлежит ей самой: не включив её в полосу, мы просим поверхность
     меньше дока — и он оказывается обрезан снизу. Именно так и вышло: панель
     выглядела подрезанной, а меню Пуск не помещалось вовсе. */
  const своя = window.Поверхности && Поверхности.панель();
  if (!своя && innerHeight - r.bottom > 4) return;
