...
/* какие программы вообще есть на машине
   ...
   Ответ «есть» помним навсегда: установленная программа никуда не денется.
   А вот ответ «нет» помним недолго — полминуты.

   Это стоило целой поломки. Человек ставит программу — из Магазина,
   кнопкой в «Параметрах», да хоть из терминала, — а агент до конца сеанса
   продолжает отвечать, что её нет: он запомнил это в первый раз, когда её
   и правда не было. Со стороны выглядело так, будто установка не сработала
   вовсе. */
const have = {};
const КОГДА = {};
const ЗАБЫТЬ = 30000;
export async function has(cmd){
  const было = have[cmd];
  if (было === true) return true;
  if (было === false && Date.now() - (КОГДА[cmd] || 0) < ЗАБЫТЬ) return false;
  try { await run('which', [cmd]); have[cmd] = true; }
  catch(e){ have[cmd] = false; КОГДА[cmd] = Date.now(); }
  return have[cmd];
}
...
const ЗНАЧКИ_КОРНИ = [
  ...
  /* Значки программ, которых на машине ещё нет.
  
     apt вместе со списками пакетов скачивает и описания программ для
     магазинов — DEP-11, — а к ним каталог значков. Он лежит здесь и уже
     оплачен: те же файлы читает GNOME Software, чтобы показывать лица
     программ до установки. Нам это даёт значки без единого похода в
     сеть — и они верные, из того же выпуска Ubuntu, что и сам пакет. */
  ...
  '/opt',
  '/usr/share/icons',
  /* Старое общее место для значков. По нынешним правилам его нет, но им
     пользуется половина программ Debian и Ubuntu: LibreOffice, Java, сам
     python — все кладут значок сюда. Без него у них нет лица. */
  '/usr/share/pixmaps'
];
...
async function собери_значки(){
  ...
  const положи = путь => {
    ...
    /* В каталоге от apt имена составные: «vlc_vlc.png», «gimp_gimp.png» —
       сперва пакет, потом сам значок. Кладём и то и другое по отдельности:
       спрашивать у нас будут по имени пакета. */
    if (имя.includes('_')){
      const [пакет, остаток] = [имя.slice(0, имя.indexOf('_')), имя.slice(имя.indexOf('_') + 1)];
      if (пакет) положиПод(пакет.toLowerCase());
      if (остаток) положиПод(остаток.toLowerCase());
    }
  };
  ...
  return {
        ...
        for (const f of await readdir(dir).catch(() => [])){
          ...
          try {
            ...
            /* Читаем только группу [Desktop Entry] — саму программу.
               ...
               «Библиотека», «Друзья», у Firefox — «Новое окно». У каждого
               своё Name и свой перевод. Разбор шёл по всему файлу подряд,
               и если у самой программы русского имени нет, а у подменю
               есть, программа получала имя подменю: Steam в меню назывался
               «Магазин», вытеснял наш Магазин и выглядел как он.

               Группа кончается там, где начинается следующая. */
            const начало = весь.search(/^\[Desktop Entry\]\s*$/m);
            const после = начало < 0 ? '' : весь.slice(начало + 1);
            const конец = после.search(/^\[[^\]]+\]\s*$/m);
            const t = начало < 0 ? весь : (конец < 0 ? после : после.slice(0, конец));
    ...
    async 'sys.icon'({ имя, ярлык }){
      ...
      if (ключ.startsWith('/') && existsSync(ключ)) путь = ключ;
        ...
        /* Имя у значка бывает длиннее имени программы: у LibreOffice на
           диске лежат libreoffice-calc, libreoffice-writer и прочие, а
           просто «libreoffice» нет. Берём ближайшее по началу — самое
           короткое из подходящих, оно же обычно и самое общее. */
        if (!путь){
          for (const в of варианты){
            if (!в || в.length < 3) continue;
            let лучшее = null;
            for (const ключ of карта.keys()){
              if (!ключ.startsWith(в + '-') && !ключ.startsWith(в + '_')) continue;
              if (!лучшее || ключ.length < лучшее.length) лучшее = ключ;
            }
            if (лучшее){ путь = карта.get(лучшее).путь; break; }
          }
        }
      }
    ...
    async 'sys.launch'({ id }){
      ...
      /* Запускаем сами, а не через gio, по двум причинам.

         Первая: gio раньше звался через call(), а тот отдаёт команде
         окружение самого агента. Агент поднимается раньше оконного сервера
         и про экран ничего не знает, поэтому программа не получала ни
         DISPLAY, ни WAYLAND_DISPLAY — и умирала молча, не показав окна.
         Значки в доке и Пуске не открывали ничего, хотя из терминала те же
         программы работали: терминал окружение получал, а они нет.

         Вторая: gio завершается успешно сразу, как только попросил
         программу открыться, и что с ней стало дальше, он не знает. Мы
         говорили человеку «откроется своим окном» и в случае неудачи
         молчали. Запуская сами, мы полторы секунды смотрим, жива ли она, и
         если нет — показываем её последнюю строку ошибки.

         Берём строку Exec, убираем подстановки вроде %U и запускаем первую
         команду с её доводами. Ничего не разбирается через оболочку: список
         доводов собирается здесь. */
      ...
      /* Ярлык без разбираемой строки запуска или требующий терминала: тут
...
export function tray(allowLaunch){
  ...
  return {
    ...
    async 'sys.tray.закрыть'({ служба, 'имя':подпись }){
      if (!allowLaunch) throw new Error('лоток выключен: запустите агент с ключом --allow-launch');
      if (!/^[\w.:-]+$/.test(String(служба || ''))) throw new Error('неверное имя программы');
      ...
      /* Кто хозяин этого процесса — знает /proc, и спросить надо до
         попытки, а не после.

         Человек получал «программа не закрылась: kill EPERM». EPERM — это
         не поломка, это отказ: закрыть чужой процесс нельзя, и никакая
         оболочка этого не сможет. Но по такому сообщению понять нечего, и
         выглядит оно как наша беда. Называем и программу, и хозяина, и
         говорим, что делать. */
      const прочти = async что => {
        try { return await readFile('/proc/' + pid + '/' + что, 'utf8'); }
        catch(e){ return ''; }
      };
      const состояние = await прочти('status');
      const имяПрог = (состояние.match(/^Name:\s*(.+)$/m) || [])[1] || 'Программа';
      const чейUid = +((состояние.match(/^Uid:\s*(\d+)/m) || [])[1]);
      const свой = typeof process.getuid === 'function' ? process.getuid() : чейUid;

      /* Шина назвала номер, которого быть не может.

         Человек закрывал программу из лотка и получал: «„oom_reaper“
         запущена от имени root». oom_reaper — не программа вовсе, а нить
         ядра: у неё нет ни окна, ни значка, ни строки запуска. Значит,
         номер пришёл не тот — такое бывает, когда за программу на шине
         отвечает посредник или номер уже переиспользован. Верить такому
         ответу нельзя, а убивать по нему — тем более.

         Ищем тогда по имени, и только среди своих процессов: имя значка в
         лотке обычно и есть имя программы. Если под него подходит больше
         одного — не гадаем. */
      const строкаЗапуска = (await прочти('cmdline')).replace(/\0/g, ' ').trim();
      if (!строкаЗапуска){
        const кого = String(подпись || '').trim();
        const годится = кого.length >= 3 && /^[\w .@+-]+$/u.test(кого);
        if (годится){
          const найдено = await run('pgrep', ['-u', String(свой), '-f', кого])
            .then(r => String(r.stdout).trim().split('\n').filter(Boolean))
            .catch(() => []);
          const чужие = найдено.map(Number).filter(n => n && n !== process.pid);
          if (чужие.length === 1){
            try { process.kill(чужие[0], 'SIGTERM'); return { ok:true, pid:чужие[0], 'поИмени':true }; }
            catch(e){ /* не вышло — расскажем ниже общими словами */ }
          }
        }
        throw new Error('Система назвала не ту программу: под этим значком '
          + 'скрывается служебный процесс ядра, а настоящую найти по имени не вышло. '
          + 'Закройте программу из неё самой — щёлкните значок в лотке.');
      }

      if (Number.isFinite(чейUid) && чейUid !== свой){
        const кто = чейUid === 0 ? 'root' : ('пользователя с номером ' + чейUid);
        throw new Error(`«${имяПрог}» запущена от имени ${кто}, а оболочка работает `
          + 'от вашего имени — закрыть её отсюда нельзя. Закройте её из самой '
          + 'программы: щёлкните значок в лотке или выберите «Меню программы».');
      }

      try { process.kill(pid, 'SIGTERM'); }
      catch(e){
        if (e && e.code === 'EPERM')
          throw new Error(`«${имяПрог}» не отдаёт себя закрыть: у оболочки нет на это `
            + 'прав. Так бывает у программ, которые запускает система, а не человек. '
            + 'Закройте её из самой программы.');
        throw new Error('программа не закрылась: ' + (e && e.message ? e.message : e));
      }
      return { ok:true, pid, 'имя':имяПрог };
    }
  };
}
...
export function shots(allowLaunch){
  ...
  return {
    ...
    /* Проба выделения области — для «врача».

       Человек жмёт Win+Shift+S, получает наше «выделите область» и дальше
       не происходит ничего: рамки нет, курсор обычный. Понять с той
       стороны, работает ли slurp вообще, было нечем — а он либо не
       запускается вовсе, либо запускается и молча выходит.

       Запускаем его по-настоящему, но на полторы секунды, и смотрим:
       если он к этому сроку ещё жив — значит, рамку он рисует и ждёт
       человека, беда не в нём. Если вышел сам — показываем, с чем. */
    async 'sys.shot.проба'(){
      const среда = await средаЭкрана();
      const есть = { grim:await has('grim'), slurp:await has('slurp') };
      const ответ = { среда:{ WAYLAND_DISPLAY:среда.WAYLAND_DISPLAY || '(нет)',
                              XDG_RUNTIME_DIR:среда.XDG_RUNTIME_DIR || '(нет)',
                              DISPLAY:среда.DISPLAY || '(нет)' },
                      есть, 'вышел':null, 'жалоба':'', 'ждёт':false };
      if (!есть.slurp) return ответ;

      const { spawn } = await import('node:child_process');
      const дитя = spawn('slurp', [], { env:среда, stdio:['ignore', 'ignore', 'pipe'] });
      let жалоба = '';
      дитя.stderr.on('data', д => { жалоба += String(д); });
      const итог = await new Promise(готово => {
        const часы = setTimeout(() => готово({ 'ждёт':true }), 1500);
        дитя.on('exit', код => { clearTimeout(часы); готово({ 'вышел':код }); });
        дитя.on('error', е => { clearTimeout(часы); готово({ 'вышел':-1, 'жалоба':String(е.message) }); });
      });
      try { дитя.kill('SIGTERM'); } catch(e){}
      ответ['ждёт'] = !!итог['ждёт'];
      ответ['вышел'] = итог['вышел'] == null ? null : итог['вышел'];
      ответ['жалоба'] = (жалоба + ' ' + (итог['жалоба'] || '')).trim().slice(0, 200);
      return ответ;
    },
        ...
        if (подWayland){
          ...
          if (режим === 'область'){
            ...
            /* slurp рисует рамку поверх всего и ждёт человека. Пока он ждёт,
               ...
               А вот если он не смог даже начать — это поломка, и молчать о
               ней нельзя. Раньше здесь ловилась любая беда и называлась
               «отменено»: человек жал Win+Shift+S, не происходило ровно
               ничего, и узнать почему было негде. Отказ отличаем от отмены
               по тому, что slurp сказал в stderr: отменивший молчит. */
            let область_ошибка = null;
            const { stdout } = await run('slurp', [], { timeout:120000, env:среда })
              .catch(e => { область_ошибка = e; return { stdout:'' }; });
            область = String(stdout).trim();
            if (!область){
              const жалоба = String((область_ошибка && область_ошибка.stderr) || '').trim();
              if (жалоба) throw new Error('не вышло выделить область: ' + жалоба.slice(0, 200));
              return { ok:false, 'отменено':true };
            }
          }
