  ...
  return {
    ...
    async 'login.enter'({ имя, пароль, сеанс }){
      ...
      try {
        ...
        /* Сеанс пошёл — экрану входа пора уйти. Ждать, пока служба входа
           сама снимет его, нельзя: пока наш оконный сервер держит экран,
           рабочий стол на него не попадёт, и человек после пароля смотрит
           на тот же самый экран входа. Сигнал уходит всей нашей группе
           процессов — и оболочке, и оконному серверу, и этому агенту, —
           но только после того, как ответ дойдёт до страницы. */
        setTimeout(() => { try { process.kill(0, 'SIGTERM'); } catch(e){} }, 600).unref();
