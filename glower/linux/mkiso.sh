...
# машин остаётся без сети, а «свободный образ» оборачивается неработающим
# ноутбуком.
#
...
#
# grim и slurp снимают экран под Wayland, maim и xclip — под X. Они не
# заменяют друг друга: первые говорят по одному протоколу, вторые по
# другому, и на машине без KMS сеанс идёт под X, где первые бесполезны.
# Кладём обе пары, чтобы «Ножницы» работали на любой машине.
#
# Telegram в образ не кладём, хотя и хотели. В архиве Ubuntu 24.04 пакета
# telegram-desktop больше нет, а оставшиеся пути плохи каждый по-своему:
# сборка с Flathub тянет за собой общую основу и добавляет к образу сотни
# мегабайт, упирая его в предел, который GitHub ставит на файл релиза; а
# архив с сайта не содержит файла значка, и программа осталась бы в меню
# без лица. Поэтому Telegram ставится из магазина одним нажатием — оттуда
# он приходит со значком и обновляется сам.
#
...
# Необязательные программы ставим отдельно и поштучно. Причина простая и
# дорого выученная: apt при одном ненайденном имени отменяет установку
# целиком. Стоило добавить в общий список telegram-desktop, которого в
# архиве Ubuntu 24.04 больше нет, — и в образ не попало ничего вообще, а
# сборка развалилась дальше по цепочке, в месте, никак с Telegram не
# связанном. Приятная мелочь не должна ронять систему.
# Обои GNOME — настоящие, из их же пакета: тридцать мегабайт картинок,
# которые человек иначе не получит никак. Набор KDE вдвое тяжелее девяноста
# мегабайт, и класть его в каждый образ — плохая сделка: его предлагают
# доставить одной кнопкой в «Параметрах», уже с машины.
# steam-devices — двадцать килобайт правил udev для геймпадов. Сам Steam
# из песочницы их поставить не может и при первом запуске жалуется окном
# по-английски; человеку, который просто поставил игру, это непонятно.
for extra in gnome-backgrounds steam-devices; do
  apt-get install -y --no-install-recommends "$extra" >/dev/null 2>&1 \
    && echo "  доп.: $extra — поставлен" \
...
# Такой же посредник для клавиши Win: она открывает меню программ. Клавиши
# достаются окну в фокусе, поэтому Win держит оконный сервер, а не страница
...
# --------------------------------------------------------------------------
# Драйверы сетевых карт, которых нет в ядре.
#
# Большинство карт ядро знает само, а прошивки к ним лежат в linux-firmware —
# он уже стоит. Но есть карты, драйверы к которым в ядро не входят: их
# держат отдельно, потому что они закрытые или просто не приняты. Такая
# карта выглядит для системы как отсутствующая, и человек читает
# «беспроводных устройств нет», глядя на живую железку.
#
# Ставим их по одному и не считаем неудачу поводом остановить сборку: эти
# драйверы собираются под конкретное ядро, и если какой-то из них однажды
# перестанет собираться, образ всё равно нужен — на других картах он не
# нужен вовсе. Заголовки ядра оставляем в системе намеренно: после
# обновления ядра драйверы должны пересобраться сами, иначе Wi-Fi отвалится
# ровно в тот день, когда человек нажмёт «Обновить».
apt-get install -y --no-install-recommends linux-headers-generic dkms || true

# Broadcom. BCM43142 и родня — единственные карты, к которым не подходит
# ни один открытый драйвер. При этом Bluetooth в той же железке работает
# через общий драйвер, и со стороны кажется, что Wi-Fi-карты нет вовсе.
# Стоит в множестве ноутбуков Lenovo и HP тех лет.
#
# Realtek. Карты 8821CE, 8812AU, 88x2BU в ядро не вошли: в мейнлайне для
# них либо ничего нет, либо есть драйвер, который их не узнаёт. Очень
# частые — и внутри ноутбуков, и в виде USB-свистков.
#
# Broadcom b43. Старые карты, где сам драйвер в ядре есть, а прошивку
...
for drv in bluez-firmware \
               ...
               firmware-b43legacy-installer; do
  if apt-get install -y --no-install-recommends "$drv" >/dev/null 2>&1; then
    echo "  драйвер $drv: встал"
  else
    echo "  драйвер $drv: не поставился — карты этого вида останутся без сети"
  fi
done

# Модемы и телефон вместо модема. Сюда попадают USB-свистки операторов,
# встроенные модемы ноутбуков и обычный телефон, раздающий интернет по
# кабелю: NetworkManager умеет с ними работать, но только если рядом есть
# ModemManager и переключатель режимов — иначе свисток так и останется
# виртуальным приводом с драйверами для Windows.
apt-get install -y --no-install-recommends \
  modemmanager usb-modeswitch usb-modeswitch-data libmbim-utils libqmi-utils || \
  echo "  поддержка модемов не встала"
